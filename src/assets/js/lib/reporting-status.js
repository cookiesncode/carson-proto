...
function bindTemplate($template, feedItem) {
  $template.find('.item_img').attr('src',feedItem.imgSrc);
  $template.find('.item_url').attr('href', feedItem.url).text(feedItem.title);
  return $template;
}
var $newCellTemplate = $('.feeds .template');
$.getJSON(feedUrl, function (data, textStatus, jqxhr) {
  if (textStatus === 'success') {
    $.each(data, function(key, val) {
      var $newCell = $newCellTemplate.clone().removeClass('template hide');
    ...
    $newCellTemplate.next('.loading-elem').remove();
  }
  else {
